...
void error_type_mismatch(const string& varName, int t1, int t2) {
	cerr << "ERROR: Type mismatch. Variable or literal '" << varName 
		<< "' of type '" << typemap_get(t1) 
		<< "' does not match expected type '" << typemap_get(t2) 
		<< "'" << endl;
}
...
void error_type_mismatch_cond(int t1) {
	cerr << "ERROR: Type mismatch. Conditional expected a predicate, but "
		<< "got an expression of type " << typemap_get(t1) << endl;
}
